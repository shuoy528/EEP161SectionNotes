...
fe_model <- feols(CornYield ~ gdd + edd + ppt + I(ppt^2) | 
                  Year + GEOID[Year], data = corn_data)
summary(fe_model)
```

To understand how fixed effects work:

-   Run a fixed effects regression for corn yield, EDD, and precipitation, removing county (GEOID), year (Year) effects and county-specific time trend.
...
Restricting to counties east of the 100th meridian ensures a more homogenous sample, reducing noise caused by irrigation, land use, and climate variability.
